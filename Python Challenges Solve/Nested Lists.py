...
Print the name(s) of any student(s) having the second-lowest grade in. If there are multiple students, order their
...
if __name__ == '__main__':
    ...
    for x in range(len(students_list)):
        if m == students_list[x][1]:
            pass
        else:
            second = students_list[x][1]
            new_student_l.append(students_list[x])
            if new_student_l[0][1] != second:
                new_student_l.pop()
                break

    new_student_l.sort(key=lambda x: x[0])

    for i in range(len(new_student_l)):
        print(new_student_l[i][0])